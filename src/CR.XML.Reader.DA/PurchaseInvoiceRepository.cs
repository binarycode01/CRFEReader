...
namespace CR.XML.Reader.DA
{
    ...
    {
        #region Constructor
        public PurchaseInvoiceRepository(IDbConnection connection, ILogger<PurchaseInvoiceRepository> logger) : base(connection, logger) { }
        #endregion
        ...
        private void Header(FacturaElectronicaCompra entity)
        {
            this.Connection.Execute(string.Format(Query.InsertDocument, this.TableName), new
            {
                ...
                CondicionVenta = EnumTools.GetXmlAttributeValue<FacturaElectronicaCompraCondicionVenta>(entity.CondicionVenta),
                PlazoCredito = entity.PlazoCredito
            });
        }

        private void PaymentMethod(FacturaElectronicaCompra entity)
        {
            HashSet<string> payMethod = new HashSet<string>();

            foreach (var item in entity.MedioPago)
            {
                if (payMethod.Contains(EnumTools.GetXmlAttributeValue<FacturaElectronicaCompraMedioPago>(item)))
                    continue;

                payMethod.Add(EnumTools.GetXmlAttributeValue<FacturaElectronicaCompraMedioPago>(item));

                this.Connection.Execute(string.Format(Query.InsertDocumentPaymentMethod, this.TableName), new
                {
                    Clave = entity.Clave,
                    MedioPago = EnumTools.GetXmlAttributeValue<FacturaElectronicaCompraMedioPago>(item)
                });
            }
        }

        private void Detail(FacturaElectronicaCompra entity)
        {
        ...
        private void OtherCharges(FacturaElectronicaCompra entity)
        {
        ...
        private void Totals(FacturaElectronicaCompra entity)
        {
        ...
        private void References(FacturaElectronicaCompra entity)
        {
            ...
            foreach (var item in entity.InformacionReferencia)
            {
                ...
                {
                    entity.Clave,
                    TipoDoc = EnumTools.GetXmlAttributeValue<FacturaElectronicaCompraInformacionReferenciaTipoDoc>(item.TipoDoc),
                    item.Numero,
                    item.FechaEmision,
                    Codigo = EnumTools.GetXmlAttributeValue<FacturaElectronicaCompraInformacionReferenciaCodigo>(item.Codigo),
                    item.Razon
                });
            }
        }

        private void OtherText(FacturaElectronicaCompra entity)
        {
        ...
        private void OtherContent(FacturaElectronicaCompra entity)
        {
